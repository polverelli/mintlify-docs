...
The response includes an `exchangeRateId` and `expiresAt`. Use this ID to execute the on/off-ramp **before it expires**; attempts after expiry will be rejected. If you don't need guaranteed pricing, use a [**floating exchange rate**](/guides/locked-exchange-rate) instead.
...
<Steps>
  <Step title="Request a locked rate">
    Ask for a guaranteed rate with a specified lock duration.
  </Step>
...
You can pass a `partnerFeeId` to bake partner fees into the quote.

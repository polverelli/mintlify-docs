...
description: "Convert fiat to stablecoin with floating or locked FX rates"
---
...
2. **Exchange Rate (optional)** — Choose a [floating](/guides/floating-exchange-rate) or [locked](/guides/locked-exchange-rate) quote.
3. **Partner Fee (optional)** — Configure custom fees ([see partner fees](/concepts/partner-fees)).
...
| `processing`     | Payment confirmed; conversion and delivery in progress.                                                |
| `success`        | Stablecoin delivered; `state.receipt.*` (rate/amount/fees) and, when applicable, `state.blockchain.*`. |
...
- Explore [**Off-Ramp Transactions**](/guides/off-ramp-transactions) to convert stablecoins to fiat